...
export const DitheringPanel = ({ settings, updateSetting, paletteData, onPaletteAction }) => {
    ...
    return (
        <PanelSection title="Mixing / Dithering">
            ...
            {settings.ditherCategory !== 'analytical' && <div className="pt-1"><div className="flex justify-between slider-label mb-1.5"><span>INTENSITY / SPREAD</span><span>{Math.round(settings.dithering * 100)}%</span></div><RangeSlider min={0} max={1} step={0.05} value={settings.dithering} onChange={(e) => updateSetting('dithering', Number(e.target.value))} /></div>}
        </PanelSection>
    );
};
